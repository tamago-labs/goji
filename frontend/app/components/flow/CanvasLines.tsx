...
export default function CanvasLines({ cards, connections, onDeleteConnection }: CanvasLinesProps) {
  ...
  return (
    <svg className="absolute top-0 left-0 pointer-events-none" style={{ overflow: "visible", width: 4000, height: 4000 }}>
      <defs>
            ...
            {conn.label && (
              <g>
                <rect
                  ...
                  strokeWidth={1}
                />
                <text x={midX} y={midY + 4} textAnchor="middle" fill="#C97A3D" fontSize={10} fontFamily="monospace">
                  {conn.label} USDC
                </text>
              </g>
            )}
