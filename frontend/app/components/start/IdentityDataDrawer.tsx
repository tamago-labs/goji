...
export default function IdentityDataDrawer({ open, apiUrl, wallet, pass, existing, onClose, onSaved }: { open: boolean; apiUrl: string; wallet: WalletData | null; pass: PassInfo | null; existing: IdentityRecord | null; onClose: () => void; onSaved: () => void }) {
  ...
  const countryField = (key: 'country', label: string) => <label className='block text-xs text-ink/45'>{label}<select value={form[key]} onChange={(event) => set(key, event.target.value)} className='mt-1.5 w-full rounded-xl border border-ink/10 bg-ink/5 px-3 py-2.5 text-sm outline-none'><option value=''>Select country</option>{COUNTRIES.map(([code, name]) => <option key={code} value={code}>{name} ({code})</option>)}</select></label>
  async function save() {
