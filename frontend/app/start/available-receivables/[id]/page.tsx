                  ...
                  <div key={flow.id} className='bg-ink/[0.02] rounded-xl overflow-hidden'>
                    ...
                    {expandedFlow === flow.id && (
                      <div className='px-4 pb-3 border-t border-ink/5'>
                        ...
                        {/* Document Preview */}
                        {selectedDocument === flow.id && (
                          <div className='mt-3'>
                            <DocumentPreview
                              templateId={flow.template}
                              customDoc={flow.customDoc}
                              flow={flow}
                              apiUrl={apiUrl}
                            />
                          </div>
...
function DocumentPreview({ templateId, customDoc, flow, apiUrl }: {
...
<div class="footer">Generated by Goji</div>
        ...
        if (customDoc) {
          try {
            customFields = JSON.parse(customDoc)
          } catch {}
        }
    ...
    load()
  }, [templateId, customDoc, flow, apiUrl])
  ...
  if (!html) {
    return (
      <div className='text-center py-4 text-xs text-ink/40'>
