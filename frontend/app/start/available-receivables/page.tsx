...
import { getTokenStatusLabel, getTokenStatusColor } from '../../../lib/receivableToken'
...
export default function AvailableReceivablesPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'active': return <span className='text-[10px] font-medium px-2 py-0.5 rounded-full bg-blue-100 text-blue-700'>Active</span>
      case 'funded': return <span className='text-[10px] font-medium px-2 py-0.5 rounded-full bg-mint/15 text-[#1B7A50]'>Funded</span>
      case 'expired': return <span className='text-[10px] font-medium px-2 py-0.5 rounded-full bg-amber-100 text-amber-700'>Expired</span>
      case 'redeemed': return <span className='text-[10px] font-medium px-2 py-0.5 rounded-full bg-mint/15 text-[#1B7A50]'>Redeemed</span>
      default: return <span className='text-[10px] font-medium px-2 py-0.5 rounded-full bg-ink/10 text-ink/50'>{status}</span>
    }
  }
          ...
          <table className='w-full text-sm'>
            <thead>
              <tr className='border-b border-ink/5 text-left'>
                ...
                <th className='px-6 py-2 text-[10px] text-ink/40 uppercase tracking-wider font-medium'>Proofs</th>
            ...
            <tbody>
              {filtered.map((r) => (
                <tr key={r.id} className='border-b border-ink/5 hover:bg-ink/3 transition-colors'>
                  ...
                  <td className='px-6 py-3 text-ink/40 text-xs'>{r.proofs.length}</td>
                  <td className='px-6 py-3'>
                    <Link
                      href={`/start/due-diligence?token=${r.tokenAddress}`}
