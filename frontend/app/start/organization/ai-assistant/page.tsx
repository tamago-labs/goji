...
export default function AIAssistantPage() {
  ...
  if (health?.role !== 'company') return <div className='rounded-2xl bg-card p-8 text-center text-sm text-ink/50'>Only the company administrator can manage the private knowledge base.</div>
  ...
  return (
    <div>
      ...
      <div className='mb-5 flex items-center gap-3 rounded-2xl bg-card p-4 shadow-[0_4px_20px_rgba(43,36,64,0.05)]'><span className={`h-2.5 w-2.5 rounded-full ${model?.status === 'ready' ? 'bg-[#42B883]' : model?.status === 'loading' ? 'animate-pulse bg-amber-400' : 'bg-ink/20'}`} /><div><p className='text-sm font-medium text-ink'>{model?.status === 'ready' ? 'Embedding model ready' : model?.status === 'loading' ? 'Loading embedding model...' : 'Embedding model stopped'}</p><p className='text-xs text-ink/35'>GTE-Large runs on the company terminal; authorized members search via encrypted P2P.</p></div></div>
