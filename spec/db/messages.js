...
const encoding19 = {
  preencode(state, m) {
    ...
    if (version >= 2 && m.companyName) c.string.preencode(state, m.companyName)
    if (version >= 2 && m.fields) c.json.preencode(state, m.fields)
    c.string.preencode(state, m.html)
    if (version >= 2 && m.isDefault) c.int.preencode(state, m.isDefault)
    if (version >= 2 && m.createdBy) c.buffer.preencode(state, m.createdBy)
    c.int.preencode(state, m.createdAt)
    c.int.preencode(state, m.updatedAt)
  },
  encode(state, m) {
    const flags =
      ((version >= 2 && m.companyName) ? 1 : 0) |
      ((version >= 2 && m.fields) ? 2 : 0) |
      ((version >= 2 && m.isDefault) ? 4 : 0) |
      ((version >= 2 && m.createdBy) ? 8 : 0)

    c.buffer.encode(state, m.id)
    c.string.encode(state, m.name)
    c.uint.encode(state, flags)

    if (version >= 2 && m.companyName) c.string.encode(state, m.companyName)
    if (version >= 2 && m.fields) c.json.encode(state, m.fields)
    c.string.encode(state, m.html)
    if (version >= 2 && m.isDefault) c.int.encode(state, m.isDefault)
    if (version >= 2 && m.createdBy) c.buffer.encode(state, m.createdBy)
    c.int.encode(state, m.createdAt)
    c.int.encode(state, m.updatedAt)
  },
  decode(state) {
    ...
    return {
      id: r0,
      name: r1,
      companyName: (version >= 2 && (flags & 1) !== 0) ? c.string.decode(state) : null,
      fields: (version >= 2 && (flags & 2) !== 0) ? c.json.decode(state) : null,
      html: c.string.decode(state),
      isDefault: (version >= 2 && (flags & 4) !== 0) ? c.int.decode(state) : 0,
      createdBy: (version >= 2 && (flags & 8) !== 0) ? c.buffer.decode(state) : null,
      createdAt: c.int.decode(state),
      updatedAt: c.int.decode(state)
    }
  }
