...
const encoding6 = {
  preencode(state, m) {
    ...
    if (m.proof) c.buffer.preencode(state, m.proof)
    c.int.preencode(state, m.createdAt)
    if (version >= 2 && m.identityPublicKey) c.buffer.preencode(state, m.identityPublicKey)
  },
  encode(state, m) {
    const flags =
      (m.chainType ? 1 : 0) |
      ...
      (m.name ? 4 : 0) |
      (m.proof ? 8 : 0) |
      ((version >= 2 && m.identityPublicKey) ? 16 : 0)
    ...
    if (m.proof) c.buffer.encode(state, m.proof)
    c.int.encode(state, m.createdAt)
    if (version >= 2 && m.identityPublicKey) c.buffer.encode(state, m.identityPublicKey)
  },
  decode(state) {
    const r0 = c.buffer.decode(state)
    const r1 = c.string.decode(state)
    ...
    return {
      ...
      identityPublicKey: (version >= 2 && (flags & 16) !== 0) ? c.buffer.decode(state) : null
    }
  }
}
